...
A single-file version for <https://live.lean-lang.org/>.

Paste this entire file into Lean4Web.  The four definitions used from
Formal Conjectures are reproduced below, so no local modules are required.
...
Lean4Web's "Latest Mathlib" project is a moving target.  The `#eval` command
at the end prints the actual Lean version used by the current session.
-/
...
lemma exists_triangleFree_subgraph_degree_ge_indepNeighbors (G : SimpleGraph α) :
  ...
  have hIG : ∀ x ∈ I, G.Adj v x := by
    ...
    obtain ⟨y, hy, rfl⟩ := hx
    exact y.property
  have hvI : v ∉ I := fun hv => (hIG v hv).ne rfl
...
lemma leafCount_eq_two_add_sum_degree_sub_two
  ...
  have hpos : ∀ v, 0 < T.degree v := by
    ...
    exact hT.1.preconnected.exists_adj_of_nontrivial v
  have hpoint : ∀ v,
      (T.degree v : ℤ) =
        2 + ((T.degree v - 2 : ℕ) : ℤ) -
